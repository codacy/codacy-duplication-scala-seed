...
object Implicits {
  ...
  implicit lazy val duplicationCloneFileWrites: Writes[DuplicationResult.CloneFile] =
    Json.writes[DuplicationResult.CloneFile]
  ...
  private def getType[T](implicit evidence: reflect.ClassTag[T]): String = {
    evidence.runtimeClass.getName
  }

  implicit val analysisProblemReasonWrites: OWrites[Problem.Reason] = {
  ...
  implicit val duplicationResultWrites: OWrites[DuplicationResult] = {
    val duplicationResultICloneWrites: OWrites[DuplicationResult.Clone] = Json.writes[DuplicationResult.Clone]
    val duplicationResultProblemWrites: OWrites[DuplicationResult.Problem] = Json.writes[DuplicationResult.Problem]

    OWrites[DuplicationResult] {
      case v: DuplicationResult.Clone   => addType[DuplicationResult.Clone](duplicationResultICloneWrites.writes(v))
      case v: DuplicationResult.Problem => addType[DuplicationResult.Problem](duplicationResultProblemWrites.writes(v))
    }
  }
  ...
  implicit lazy val duplicationToolCodacyConfigurationFormat: Reads[DuplicationTool.CodacyConfiguration] =
    Json.reads[DuplicationTool.CodacyConfiguration]
